...
interface TestBalanceQueryRow {
  account_code: string;
  total_debit: string | number;
  total_credit: string | number;
}

export interface TestBalanceAccountRow {
  type: string;
  p_balance: number;
  c_balance: number;
  difference: number;
...
export class TestBalanceService {
  ...
  async getTestBalance(month: string = 'June', year: number = 2026) {
    ...
    const dbRes = await this.dataSource.query<TestBalanceQueryRow[]>(query, [
      periodStr,
      altPeriodStr,
    ]);
    ...
    const accounts: TestBalanceAccount[] = [
      {
        code: '210001',
        name: 'ACCOUNT PAYABLE - 210001',
        type: 'AP',
        status: 'AP Active',
        bg_balance: -50242296.0,
        rows: [
          { type: 'IN', p_balance: -4292432.19, c_balance: -4292432.19, difference: 0.0 },
          { type: 'CR', p_balance: 0.0, c_balance: 0.0, difference: 0.0 },
          { type: 'CK', p_balance: 267.0, c_balance: 267.0, difference: 0.0 },
          { type: 'JE', p_balance: 0.0, c_balance: 0.0, difference: 0.0 },
        ],
      },
      {
        code: '110019',
        ...
        rows: [
          { type: 'IN', p_balance: 67867.58, c_balance: 67867.58, difference: 0.0 },
          { type: 'CR', p_balance: -150177.27, c_balance: -150177.27, difference: 0.0 },
          { type: 'CK', p_balance: 0.0, c_balance: 0.0, difference: 0.0 },
          { type: 'JE', p_balance: 0.0, c_balance: 0.0, difference: 0.0 },
        ],
      },
      {
        ...
        type: 'AR',
        status: 'AR Active',
        bg_balance: 32068180.4,
        rows: [
          { type: 'IN', p_balance: 4746263.67, c_balance: 4746263.67, difference: 0.0 },
          { type: 'CR', p_balance: -4287244.82, c_balance: -4287244.82, difference: 0.0 },
          { type: 'CK', p_balance: 3779.26, c_balance: 3779.26, difference: 0.0 },
          { type: 'JE', p_balance: 0.0, c_balance: 0.0, difference: 0.0 },
        ],
      },
    ];
    ...
    for (const acc of accounts) {
      ...
      acc.current_balance = acc.bg_balance + sumC;
    }
