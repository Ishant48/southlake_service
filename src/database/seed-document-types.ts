...
export async function seedDocumentTypes(queryRunner: QueryRunner): Promise<void> {
  const documentTypes = [
    ...
    {
      name: 'Policy Reports - Monthly/Quarterly etc.',
      code: 'Policy Reports - Monthly/Quarterly etc.',
    },
    ...
    {
      name: 'Policy Endorsements - Second Named Insured',
      code: 'Policy Endorsements - Second Named Insured',
    },
  ];

  console.warn('Seeding Document Types...');
  for (const doc of documentTypes) {
    const exists = (await queryRunner.query('SELECT id FROM document_type_master WHERE code = $1', [
      doc.code,
    ])) as Array<{ id: string }>;
